...
bot = telebot.TeleBot('7013271030:AAG0NsD2Qr8HoP7BsjbdQP2y2XlhE9wU3h8')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: BGMI\nThanks for using our service 😎. We are not responsiblefor any of your ddos attacks. 😶‍🌫️"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                response = "Error: Time interval must be less than 299"
            else:
            ...
            response = "For example: /bgmi 127.0.0.1 8080 180"  # Updated command syntax
    else:
...
'''
    for handler in bot.message_handlers:
        if hasattr(handler, 'commands'):
            if message.text.startswith('/commands'):
                commands_text += f"{handler.commands[0]}: {handler.doc}\n"
